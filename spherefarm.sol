...
interface IFarm {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned 
to this pool. SUSHI to distribute per block.
        uint256 lastRewardBlock;  // Last block number that SUSHI 
distribution occurs.
        uint256 accSushiPerShare; // Accumulated SUSHI per share, times 
1e12. See below.
    }
    ...
    function poolInfo(uint256 pid) external view returns (IFarm.PoolInfo 
memory);
    function poolLength() external view returns (uint256);

    function userInfo(uint256 pid, address _user) external view returns 
(IFarm.UserInfo memory);

    // Return reward multiplier over the given _from to _to block.
    function getMultiplier(uint256 _from, uint256 _to) external view 
returns (uint256);
    function totalAllocPoint() external view returns (uint256);
    function emissionRate() external view returns (uint256);

    // View function to see pending CAKEs on frontend.
    function pending(uint256 pid, address _user) external view returns 
(uint256);

    // Deposit LP tokens to MasterChef for CAKE allocation.
    function deposit(uint256 pid, uint256 _amount) external;
    function deposit(uint256 pid, uint256 _amount, bool _withdrawRewards) 
external;
    function deposit(uint256 pid, uint256 _amount, address _referrer) 
external;

    // Withdraw LP tokens from MasterChef.
    function withdraw(uint256 pid, uint256 _amount) external;
    function withdraw(uint256 pid, uint256 _amount, bool _withdrawRewards) 
external;
...
contract RewardsContract is Ownable {
    ...
    constructor(address _chef, address _rewardToken, uint256 _pid, bool 
_isMiddle) {
        chef = _chef;
    ...
    function stake(uint256 _amount) public {
       ...
       if (_amount > 0) {
           poolinf.lpToken.transferFrom(msg.sender, address(this), 
_amount);
    ...
    function harvest() public {
        convertToSphere();
        UserData storage user = userInfo[msg.sender];
        uint256 pending = user.stake * accSpherePerShare  / 1e18 - 
user.rewardTally;
        user.rewardTally = user.stake * accSpherePerShare / 1e18;
        if (pending > 0) {
    ...
    function getPathForRewardToCro() private view returns (address[] 
memory) {
        address[] memory path;
        if (isMiddle) {
            path = new address[](3);
    ...
    function getPathForCroToSphere() private view returns (address[] 
memory) {
    ...
    function convertToSphere() public  {
      ...
      uint[] memory amounts = 
IMeerkatRouter02(routerTwo).swapExactTokensForETH(rewardBal, 0, 
getPathForRewardToCro(), address(this), block.timestamp);
      ...
      uint[] memory sphereOut = 
IMeerkatRouter02(mmf).swapExactETHForTokens{value: croAmount}(0,  
getPathForCroToSphere(), address(this), block.timestamp);
    ...
    function pendingReward(address _user) public view returns(uint256) {
            ...
            return user.stake * accSpherePerShare / 1e18 - 
user.rewardTally;
        }
    ...
    function tvl() public view returns(uint256) {
        IFarm.UserInfo memory vault = IFarm(chef).userInfo(pid, 
address(this));
        return vault.amount;
    }
    ...
    function setDevWallets(address payable _crip, address payable 
_interupt) external onlyOwner{
        crip = _crip;
        interrupt = _interupt;
    }
